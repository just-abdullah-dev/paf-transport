...
export async function sendVoucherToStudent(data) {
  try {
    console.log(data);

    const mailBody = `
  <br>Dear <b>${data?.name}</b>,
  <br><br>We hope this message finds you well. Below are your transport details:
  <br>
  ...
  <br>
  Thank you for using Pafiast Transport services!
  <br>
  If you have any questions, please feel free to reach out to us at <a href="mailto:${
...
`;
    ...
    await sendMail(
      data?.email,
      `${data?.name} new transport voucher has been generated.`,
      mailBody
    );
  } catch (error) {
    console.error(error);
